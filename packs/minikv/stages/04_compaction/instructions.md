...
Compaction removes operations that no longer affect recovery while preserving the same live state.
...
MiniKV writes compacted records in ascending key order. Recovery does not require that order, but a canonical artifact is easier to compare, reproduce, and inspect.

The semantic question comes first: which records belong in a result that replays to the same state? Filesystem replacement is a separate concern.
...
Replay the valid input using the `SET <key> <value>` grammar. Write one UTF-8 record for the latest value of each key:
...
- Safe replacement details for an existing destination.
