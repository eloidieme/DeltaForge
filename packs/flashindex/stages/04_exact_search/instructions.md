...
Tokenization can describe everything in the corpus. Search narrows that occurrence stream to one focused question: where does this exact name appear?
...
Under FlashIndex's token boundary rule, `main`, `main_loop`, and `domain` are three different tokens. An exact-token search should therefore return `main` and nothing else.
...
Case is part of the token text. `Request`, `request`, and `REQUEST` remain different names. FlashIndex performs exact source-name search, so it does not fold those spellings together. A tool designed for prose might reasonably offer a case-insensitive mode instead.
...
Reuse the established corpus and token rules: boundaries, case, positions, portable paths, and ordering.
