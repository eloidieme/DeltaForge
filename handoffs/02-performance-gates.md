...
- **Behavioral digest (required):** completion proofs pin a per-stage behavioral digest (`LoadedPack::stage_behavioral_digest` in `src/pack.rs`) covering tests, fixtures, and build/run commands. The moment gate definitions can block `next`, they are behaviorally relevant: include the stage's `performance_gates` (and anything they reference, e.g. the benchmark definitions they point at) in that digest so a pack update that tightens a gate marks the stage as needing revalidation. There is a `NOTE for performance gates` comment at the function marking the insertion point; update the doc comment, `docs/pack-format.md`, and add a test (gate change → `sync-pack` reports needs_revalidation).
