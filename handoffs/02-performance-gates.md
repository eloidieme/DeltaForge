...
- **Behavioral digest (required):** completion proofs pin a per-stage behavioral digest (`LoadedPack::stage_behavioral_digest` in `src/pack.rs`) covering tests, fixtures, and build/run commands. The moment gate definitions can block `next`, they are behaviorally relevant and must join that digest — but hash only the progression-affecting semantics, canonically serialized:
  - **Include** (as `serde_json::to_vec` of the parsed structs, mirroring how the digest already handles build/run commands — never raw benchmarks.yaml bytes, so YAML reformatting/comments/key order don't stale proofs): each gate's referenced benchmark name, `metric`, `min`/`max` bound, and `params` selector; the referenced benchmark definition's `command`, `fixture` reference, and `matrix`; and the language's `bench_run` command — the latter two only for stages that define gates.
  - **Exclude**: gate `advice` text (educational — editing it must behave like a documentation-only update) and measurement methodology (`iterations`, `warmup`, `timeout_ms`). Fixture *content* needs no special handling — the behavioral digest already hashes the stage's whole fixtures tree.
  - There is a `NOTE for performance gates` comment at the function spelling this out; replace it with an updated doc comment documenting the split (tests/fixtures are hashed as raw bytes because they *are* the runner's semantics; gates are hashed as parsed semantics because they have an interpretation layer). Update `docs/pack-format.md` accordingly.
  - Tests: (a) tightening a gate bound → `sync-pack` reports `needs_revalidation` and `next` blocks; (b) editing only `advice` text → proofs stay valid; (c) reformatting benchmarks.yaml without semantic change → proofs stay valid.
