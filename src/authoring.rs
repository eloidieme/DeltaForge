...
pub fn create_pack(request: &NewPackRequest) -> Result<AuthoringReport> {
    ...
    atomic_write(
        ...
        template_cargo_lock(&request.id).as_str(),
    )?;
    atomic_write(
...
/// A lockfile Cargo has nothing left to rewrite. A stub without the
/// `[[package]]` entry for the crate itself is completed by the learner's very
/// first `cargo build`, and because that write lands inside the project tree it
/// reads as a learner edit: the completion proof recorded by `deltaforge test`
/// is already stale by the time `commit`/`next` check it.
fn template_cargo_lock(id: &str) -> String {
    format!(
