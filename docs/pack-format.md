...
All six sections after `Goal` are rendered to the learner in full, as paragraphs, lists, and fenced code blocks — a section may be written as prose, as bullets, or as both, and all of it reaches the workbench. A required section that is empty is an authoring error, not a blank panel: `pack doctor` and `validate-pack --strict` report it, and a project whose current stage is missing one opens on the recovery screen naming the gap rather than failing to load.
...
`pack doctor` and `validate-pack --strict` report authoring-quality findings when any of the seven required `##` sections is absent or empty, hint headings are not numbered `1` to `n`, a stage has fewer than three hint headings, or a stage defines fewer than two tests. The section check is the same one the workbench applies before rendering a stage, so a pack these commands call clean is a pack that opens. They remain strict/doctor findings rather than base schema failures so incomplete work-in-progress packs stay editable.
