...
- `test`: run black-box stage tests and stream bounded human-readable evidence in the terminal. `--stage <id>` selects one stage; `--all` selects every stage; `--filter <text>` selects matching tests; `--list-tests` lists without running; `--fail-fast` stops the selected stage after its first failure; `--no-build` skips the build command; `--keep-temp` preserves fixture copies; `--verbose` prints full captured streams; and `--json` is the automation form.
- `explain-failure`: render the persisted structured diagnosis for the highest-priority failure, including its contract, expected and actual behavior, and fixture context. Supports `--stage <id>` and `--json`.
...
- `sync-pack`: adopt the currently discovered pack after a pack upgrade. Updates only the project-level pin (version, source, digest); completion records keep the digests of what actually passed. Reports each completed stage as valid or needing revalidation. Supports `--json`.
- `hint`: reveal progressive hints. `--level N` never lowers previously recorded progress; `--all` reveals every currently available level, but keeps the retrospective gated until the stage is acquired. Levels are numbered by position in `hints.md`, so this command and the workbench always agree about which rung a revealed level is.
...
- `bench`: run pack benchmarks. `--stage <id>` selects one stage and `--all` selects every benchmark-bearing stage. `--iterations N` and `--warmup N` override the stored measurement plan; an explicit `--warmup 0` is a diagnostic shortcut, while persisted pack and project warmups must be positive. Timing uses the pack's `bench_run` command (falling back to `run`). Human output is an aligned table per benchmark, one row per matrix point, e.g.:
  ...
  `--compare` compares each point from the new run with the most recent prior saved point having the same project, language, stage, benchmark, and exact parameters. It reports median, throughput, and peak-memory deltas with improved/regressed wording. Run once with `--save`, then run again with `--compare` (optionally also `--save`). The comparison uses the history as it existed before the new run is appended, and warns when no matching prior point exists. If the saved and current OS, architecture, or anonymized host identity differ, the output notes that the measurements may not be directly comparable. With `--json`, comparison details are attached to each output record under `comparison` and no human text is written to stdout; comparisons are never persisted.
- `report`: write the engineering record. Every line traces to a completion record, a saved measurement, a gate result, a commit, or a note the learner wrote; a project with no measurements makes no performance claim. `--format markdown|html|json`, `--output` defaults to `report.md`. Writing a report never stales the completion proof it describes: when `--output` names a plain file at the project root (the default, and the browser export's `deltaforge-report.<ext>`), that file is automatically added to the project's digest exclusions the first time it is written.
- `portfolio`: write a short narrative from the same recorded evidence. `--output` defaults to `PORTFOLIO.md`, excluded from the project digest the same way as `report`'s output.
