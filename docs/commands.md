...
- `hint`: reveal progressive hints. `--level N` never lowers previously recorded progress; `--all` reveals every currently available level, but keeps the retrospective gated until the stage is acquired.
